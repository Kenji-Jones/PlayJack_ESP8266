...
const char HTTP_PORTAL_OPTIONS[] PROGMEM  = "<form action=\"/controller\" method=\"get\"><button>CONTROLLER</button></form><form action=\"/0controller\" method=\"get\"><button>RECEIVER</button></form><br/><form action=\"/i\" method=\"get\"><button>INFO</button></form><br/>";
...
const char HTTP_FORM_PARAM[] PROGMEM      = "<form method='get' action='devicesave'><input id='{i}' name='{n}' length={l} placeholder='{p}' value='{v}' {c}><br/>";
const char HTTP_FORM_END[] PROGMEM        = "<br/><button type='submit'>save</button></form>";
const char HTTP_SCAN_LINK[] PROGMEM       = "<br/><div class=\"c\"><a href=\"/wifi\">Scan</a></div>";
const char HTTP_SAVED[] PROGMEM           = "<div style = 'font-size: 1.2em;'><span style = 'font-weight: bold;'>Saving Settings</span><br />Trying to connect device to network.<br />If it fails the <span style ='color: RED;'> RED LED</span> will blink several times. reconnect to the device's wifi to try again<br />If it connects successfully <span style ='color: green;'>GREEN LED</span> will blink several times</div>";
...
class WiFiManager
{
  ...
  private:
    ...
    boolean       _shouldBreakAfterConfig = true; // was false
